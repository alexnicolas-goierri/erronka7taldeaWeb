...
CREATE TABLE `erabiltzaile` (
  ...
  KEY `rol_id` (`rol_id`),
  CONSTRAINT `erabiltzaile_ibfk_1` FOREIGN KEY (`langile_id`) REFERENCES `langile` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `erabiltzaile_ibfk_2` FOREIGN KEY (`bezero_id`) REFERENCES `bezero` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `erabiltzaile_ibfk_3` FOREIGN KEY (`hornitzaile_id`) REFERENCES `hornitzaile` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `erabiltzaile_ibfk_4` FOREIGN KEY (`rol_id`) REFERENCES `rolak` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `erabiltzaile` VALUES (1,1,NULL,NULL,'amaia.agirre@erronka.eus','zuzendari1',1),(2,2,NULL,NULL,'jon.etxeberria@erronka.eus','jon1234',2),(3,3,NULL,NULL,'mikel.goiko@erronka.eus','mikelpass',2),(4,4,NULL,NULL,'ane.iturri@erronka.eus','anebiltegi',3),(5,5,NULL,NULL,'iker.lopez@erronka.eus','iker123',4),(6,6,NULL,NULL,'maite.zabaleta@erronka.eus','maite55',4),(7,7,NULL,NULL,'unai.bilbao@erronka.eus','unai2024',2),(8,8,NULL,NULL,'leire.mendi@erronka.eus','leiretek',2),(9,9,NULL,NULL,'gorka.uribe@erronka.eus','gorka77',3),(10,10,NULL,NULL,'saioa.arriola@erronka.eus','saioasal',4),(11,11,NULL,NULL,'aitor.garcia@erronka.eus','aitor44',2),(12,12,NULL,NULL,'nerea.azkue@erronka.eus','nerea88',2),(13,13,NULL,NULL,'ander.sanz@erronka.eus','anderbilt',3),(14,14,NULL,NULL,'julen.iriondo@erronka.eus','julen99',4),(15,15,NULL,NULL,'oihane.larrea@erronka.eus','oihane1',2),(16,16,NULL,NULL,'xabier.ortiz@erronka.eus','xabier2',2),(17,17,NULL,NULL,'irati.zabala@erronka.eus','irati3',3),(18,18,NULL,NULL,'markel.alonso@erronka.eus','markel4',4),(19,NULL,1,NULL,'cliente1@erronka.eus','cliente1',5),(20,NULL,2,NULL,'cliente2@erronka.eus','cliente2',5),(21,NULL,3,NULL,'cliente3@erronka.eus','cliente3',5),(22,NULL,4,NULL,'cliente4@erronka.eus','cliente4',5),(23,NULL,5,NULL,'cliente5@erronka.eus','cliente5',5),(24,NULL,6,NULL,'cliente6@erronka.eus','cliente6',5),(25,NULL,7,NULL,'cliente7@erronka.eus','cliente7',5),(26,NULL,8,NULL,'cliente8@erronka.eus','cliente8',5),(27,NULL,NULL,1,'proveedor1@erronka.eus','prov1',4),(28,NULL,NULL,2,'proveedor2@erronka.eus','prov2',4),(29,19,NULL,NULL,'june.vidal@erronka.eus','june55',2),(30,20,NULL,NULL,'kepa.amuriza@erronka.eus','kepa66',1);
...
INSERT INTO `rolak` VALUES (5,'Bezeroa'),(3,'Biltegizaina'),(4,'Saltzailea'),(2,'Teknikaria'),(1,'Zuzendaria');
...
-- Dump completed on 2026-02-03 10:15:04
